...
def builtModelForTest(config):
    data = initData()
    f = open('{0}_dict_{1}.pkl'.format(config['name'],str(len(data.keys()))), 'rb') 
...
def buildModelForTrain(config):
    ...
    f = open('{0}_dict_{1}.pkl'.format(config['name'],str(len(data.keys()))), 'wb')  
    ...
    for tweet in tweets:
        ...
        if len(data) == 2:
            if key=='undefined':
                ...
                key = "positive"
        if(data.has_key(key)):
            data[key].append(clean_string(tweet["text_snowball"]))
...
def saveModel(data, config, dictionary, _type):
    ...
    f = open('{0}_{1}_{2}_classes.pkl'.format(_type, config['name'], str(len(data.keys()))), 'wb')   
